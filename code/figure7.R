...
# name: figure7.R
# author: Yu Xia
# description: 2nd attempt using SA. Separate each category. 7th category.
...
res_naive_7 = feols(llar ~ i(time_til, treated, ref = -1) | 
                      fipsstat + year,
                    state_crime_cs, vcov = ~state) #What about control variables?#
summary(res_naive_7)
iplot(res_naive_7, main = "Naive TWFE")

# Sun and Abraham (SA20)
unique(state_crime_cs$treatyear)

res_cohort7 = feols(llar ~ sunab(treatyear, year) | fipsstat + year, 
                    state_crime_cs,
                    vcov = ~state)
summary(res_cohort7)
iplot(res_cohort7, ref.line = -1, main = "Sun & Abraham")

iplot(list(res_naive_7, res_cohort7), ref.line = -1,
      main = "Treatment's effect on Larceny")
...
#library(ggplot2)
ggiplot(list("TWFE" = res_naive_7, "Sun & Abraham" = res_cohort7), 
        main = "",
        xlab = "Year",
        theme = theme_minimal())
ggsave(here("figures/figure7.png", width=8, height=4.5))
