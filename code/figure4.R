...
# name: figure4.R
# author: Yu Xia
# description: 2nd attempt using SA. Separate each category. 4th category.
...
res_naive_4 = feols(lrob ~ i(time_til, treated, ref = -1) | 
                      fipsstat + year,
                    state_crime_cs, vcov = ~state) #What about control variables?#
summary(res_naive_4)
iplot(res_naive_4, main = "Naive TWFE")
...
res_cohort4 = feols(lrob ~ sunab(treatyear, year) | fipsstat + year, 
                    ...
                    vcov = ~state)
summary(res_cohort4)
iplot(res_cohort4, ref.line = -1, main = "Sun & Abraham")

iplot(list(res_naive_4, res_cohort4), ref.line = -1,
      main = "Treatment's effect on Robbery")
...
#library(ggplot2)
ggiplot(list("TWFE" = res_naive_4, "Sun & Abraham" = res_cohort4), 
        main = "",
        xlab = "Year",
        theme = theme_minimal())
ggsave(here("figures/figure4.png", width=8, height=4.5))
